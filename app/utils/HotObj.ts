...
export class HotObj {
   ...
   */
  public static hasProp<X, Y extends PropertyKey>(
    obj: X,
    prop: Y,
   ...
   */
  public static getValue<X, Y extends keyof X>(
    obj: X,
    key: Y,
  ): X[Y] | undefined {
    return obj[key];
   ...
   */
  public static extract<X, Y extends keyof X>(
    obj: X,
    keys: Y[],
  ): Expand<Pick<X, Y>> {
    const newRef = { ...obj };
   ...
   */
  public static shallowEquals<
    X extends Record<PropertyKey, unknown>,
    Y extends Record<PropertyKey, unknown>,
  >(objA: X, objB: Y): boolean {
    ...
    for (const key of keysA) {
      if (
        !this.hasProp(objB, key) ||
        !Object.is(this.getValue(objA, key), this.getValue(objB, key))
      ) {
        return false;
      }
    }
   ...
   */
  public static cleanUpNullables<Nullable extends Record<PropertyKey, unknown>>(
    obj: Nullable,
  ): NonNullable<Partial<Nullable>> {
    Object.keys(obj).map(item => {
      if (
        ["null", "undefined", "NaN", null, undefined, NaN].includes(
          <string | number | null | undefined>obj[item],
        )
      ) {
        delete obj[item];
      }
